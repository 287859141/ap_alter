...
class DatesController extends Controller {
	...
	if($_SESSION["v_username_group"]==5||$_SESSION["v_username_group"]==80){
			$where['oi.uid']= $_SESSION["v_uid_session"];
		}else{
				if($clientuser==0){
				$SelectedCuser_Array=str_replace("|",",",$SelectedCuser);
	...
	elseif(empty($begintime) && !empty($overtime)){		
		$overtime2		=strtotime($overtime);				
		$where['o.starttime'] = array('elt',$overtime2+1);
	}
	$status	;
	$orderbystr=" o.starttime ASC";
	if($status=="3") $orderbystr="oi.pid ASC";
   ...
   //die();
	...
	{ 
		global $LangStatus;
		 $cpid=$_POST['comp'];
		if(isset($LangStatus[$cpid]))
		{
			$arr=$LangStatus[$cpid]['orderStatus'];
		}
		else
		{
			$arr=$LangStatus['orderStatus'];
