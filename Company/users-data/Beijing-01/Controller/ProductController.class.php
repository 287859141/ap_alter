...
class ProductController extends Controller {
    ...
    public function index(){
		...
		$plist=$product->field("v_products.*,v_companies.name as cmpe")->join('LEFT JOIN  v_companies ON v_products.company =  v_companies.id','RIGHT RIGHT ON v_companies.name as cmpe')->where($where )->order(array('sort'=>'asc','id'=>'asc',))->select($datas);		
	...
	$where['id'] = $_SESSION["v_company_session"];	
		...
		$pslogdata=$ptlogs->field('v_products_logs.*,v_users.realname as rlname,v_products.number as ptnums,v_products.name as ptname,v_users.department as depart,v_products.prefix as prts')->join('LEFT JOIN  v_users ON v_users.id =  v_products_logs.uid LEFT JOIN  v_products ON v_products.id = v_products_logs.pid')->order("v_products_logs.plid desc")->where($where)->select();
