...
//转换普通标准时间为unix时间
function getunix($dates){
	$years=substr($dates,0,4);
	$months=substr($dates,5,2);
	$days=substr($dates,8,2);
	$hours=substr($dates,11,2);
	$mins=substr($dates,14,2);
	$secs=substr($dates,17,2);
	$date_unix=mktime($hours,$mins,$secs,$months,$days,$years);
	return $date_unix;
	}
//反过滤特殊字符
function decode_editor($content){
...
//编码v3
function encodestrall($string) {
	if(is_array($string)) {
		foreach($string as $key => $val) {
			$string[$key] = encodestrall($val);
		}
	} else {
	...
	$string=str_replace("union","u n i o n",$string);
	return ($string);
}
...
function getpage(&$m,$where,$pagesize=10){
    ...
    $p->lastSuffix=false;
    //$p->setConfig('header','<ul class="rows">共<li>%TOTAL_ROW%</li>条记录&nbsp;&nbsp;每页<li>'.$pagesize.'</li>条&nbsp;&nbsp;第<li>%NOW_PAGE%</li>页/共<li>%TOTAL_PAGE%</li>页</ul>');
    $p->setConfig('prev','上一页');
    $p->setConfig('next','下一页');
    $p->setConfig('last','末页');
    $p->setConfig('first','首页');
   $p->setConfig('theme','<ul class="page">%FIRST%<li > %UP_PAGE%</li></i> %LINK_PAGE% </li><li>%DOWN_PAGE%</li> %END% %HEADER%</ul>');
