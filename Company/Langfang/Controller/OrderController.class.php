...
class OrderController extends Controller {
    public function index(){
        ...
        $xiadingdanarr=$LangGroups['Permissions']['订购产品'];
...
$searchcount=$order->field("v_orders.*,v_companies.id as cpid,v_companies.name as cpname,v_users.company as comname,v_users.department as depart,v_users.realname as realname")->join('LEFT JOIN  v_users ON v_users.id = v_orders.uid LEFT JOIN  v_companies ON v_companies.id = v_users.company')->where($where)->count();
	...
	public function o_list_edit(){
		...
		global $LangGroups,$LandnowCom; 
		$rolelarr=$LangGroups['Permissions']['新增产品'];
		//权限过滤
	...
	public function oit_edit_del(){		
	 ...
	 public function o_edit_up(){
		...
		if($_POST){
		global $LangRolesStatus,$LangGroups;
			if(isset($LangRolesStatus[$_SESSION["v_company_session"]]['group']))
		{	
			$groupval=$LangRolesStatus[$_SESSION["v_company_session"]]['group'][$_SESSION["v_username_group"]];
		}
		else 
		{
			$groupval=$LangRolesStatus['group'][$_SESSION["v_username_group"]];
		}
		if(empty($groupval)){
			if(isset($LangRolesStatus[$_SESSION["v_company_session"]]['Admingroup'][$_SESSION["v_username_group"]]))	{
				$groupval=$LangRolesStatus[$_SESSION["v_company_session"]]['Admingroup'][$_SESSION["v_username_group"]];
			}else
			{
			$groupval=$LangRolesStatus['Admingroup'][$_SESSION["v_username_group"]];
			}
		}
	...
	public function o_cheks(){		
	 ...
	 public function o_overs(){		
		...
		global $LangRolesStatus,$LangGroups;
		$firstgroup=$LangRolesStatus['group'][81];	
		$lastgroup=$LangRolesStatus['group'][82];	
		if(!empty( $_SESSION["v_company_session"]))
		{	
			...
			{
				$firstgroup=$LangRolesStatus[$_SESSION["v_company_session"]]['group'][81];	
				$lastgroup=$LangRolesStatus[$_SESSION["v_company_session"]]['group'][82];
				}
		}
			...
			if(!empty($val['shenheuser1']))
			{
				foreach($userlastinfo as $k=>$v)
				{
					if($val['shenheuser1']==$v['id']){
						$val['laseshenhe']=$lastgroup.$v['realname'];
					break;
					}
				}
			}
	 ...
	 public function o_over_see(){
	  ...
	  public function o_status(){			
		...
		**/
		...
		$this->assign('leftmenu',$_SESSION["v_username_leftmenu"]);			
		$this->assign('order',$olist);
		$this->assign('lastgroup',$lastgroup);
		$this->assign('page',$p->show());	
        $this->display();	
	 }
	  ...
	  public function o_end_edit(){
	   ...
	   public function o_dels(){		
		...
		$where['v_users.company'] = $_SESSION["v_company_session"];	
		$lastgroup=$LangRolesStatus['group'][5];	
		if($_SESSION["v_username_group"]==5)
		{
			$where['v_orders.uid']= $_SESSION["v_uid_session"];
		}								
		...
		if(isset($LangRolesStatus[$_SESSION["v_company_session"]]['group']))
		{	
			$grouplist=$LangRolesStatus[$_SESSION["v_company_session"]]['group'];
		}else
		{
			$grouplist=$LangRolesStatus['group'];
		}
		if(isset($LangRolesStatus[$_SESSION["v_company_session"]]['Admingroup']))
		{	
			$admingouplist=$LangRolesStatus[$_SESSION["v_company_session"]]['Admingroup'];
		}else
		{
			$admingouplist=$LangRolesStatus['Admingroup'];
		}
		$delorderlist=array();
	...
	public function o_serchslist(){   //获取搜索结果
		global $LangRolesStatus,$LangGroups,$LangStatus;	
		$userwhere=array();
		if(!empty( $_SESSION["v_company_session"]))
		{			
			if(isset($LangStatus[$_SESSION["v_company_session"]]))
			{
				$orderStatusarr=$LangStatus[$_SESSION["v_company_session"]]['orderStatus'];
			}
		}
		else
		{
			$orderStatusarr=$LangStatus['orderStatus'];
		} 
	...
	if(!empty($begintime) && !empty($overtime)){
		...
		$overtime2		=strtotime($overtime+1);	
	...
	foreach($searchlist as $key=>$val){
	 ...
	 public function o_serchlist_see(){
	 	header("Content-type:text/html;charset=utf-8");	
		global $LangRolesStatus,$LangGroups,$LangStatus;	
		import("ORG.Util.Page"); 	
		$order = M("orders");	
		...
		$osendlogs = M("orders_sendlogs");				    
		$oid['v_orders.id'] = $_GET['id'];			
		$shlist=$order->field("v_orders.*,v_companies.id as cpid,v_companies.name as cpname,v_users.company as comname,v_users.department as depart,v_users.realname as realname")->join('LEFT JOIN  v_users ON v_users.id = v_orders.uid LEFT JOIN  v_companies ON v_companies.id = v_users.company')->where($oid)->find();			
		$osid['v_orderitems.oid'] = $oid['v_orders.id'];		
		$ptslist= $oitems->field("v_orderitems.*,v_orderitems.price as otprice,v_products.prefix as prefix,v_products.number as number,v_products.name as name,v_products.price as ptprice,v_products.packing_12,v_products.packing_22,v_products.packing_32")->join('LEFT JOIN v_products ON  v_orderitems.pid = v_products.id')->where($osid)->select();		
		$p=getpage($oitems,$osid,10);	
		$allprice+=$otprice;
		$this->assign('orderspct',$ptslist);		
		$this->assign('ordersee',$shlist);
		$this->assign('page',$p->show());	
		$this->assign('leftmenu',$_SESSION["v_username_leftmenu"]);	
	    $this->display();
	 }
	   public function o_serchs_edit(){
		header("Content-type:text/html;charset=utf-8");	
		global $LangRolesStatus,$LangGroups;	
		import("ORG.Util.Page"); 	
		$users = M("users");	
	...
	public function o_serchs()	{
		global $LangStatus;	
		if(isset($LangStatus[$company]))		{
			$arr=$LangStatus[$company]['orderStatus'];
		}
		else
		{
			$arr=$LangStatus['orderStatus'];
		} 
		$company = M("companies");
		$zhihangstatic=0;
		if(!empty( $_SESSION["v_company_session"]))
	...
	public function neworder(){
		...
		if($sRecord_locktime['orderdeny']==1)
		{
			...
			$overtime		=getunix($sRecord_locktime['orderover']);			
	...
	{		
		...
		{	
			...
			//减少库存 BEGIN
			$arrlist=$orderitems->field("ot.id, ot.pid,ot.amount,ot.price as otprice,p.amount as pamount,p.name,p.prefix,p.number,p.company,p.price")->join("ot LEFT JOIN v_products p ON p.id = ot.pid")->where($oitemwhere)->order("ot.id asc")->select();			
	...
	public function o_ordernew(){
		header("Content-type:text/html;charset=utf-8");	
		global $LangGroups,$LandnowCom; 
		$rolelarr=$LangGroups['Permissions']['订购产品'];
		//权限过滤
		if(in_array($_SESSION["v_username_group"],$rolelarr))
		{	
			$users=M('users');
	...
	public function o_checktime(){
		header("Content-type:text/html;charset=utf-8");	
		global $LangGroups,$LandnowCom; 
		$rolelarr=$LangGroups['Permissions']['锁定订货时间'];
